...
export function TemperamentTest({ onBack }: TemperamentTestProps) {
  ...
  const { user } = useAuth();
  const { createTest, isLoading: isSaving } = useTest();
  ...
  const determineTemperamentType = (extraversion: number, neuroticism: number) => {
    ...
    return temperamentTypes.phlegmatic;
  };
  ...
  const saveResultsToServer = async () => {
    ...
    try {
      ...
      const testData: PsychTestRequest = {
        ...
        psychParams: [
          { param: countExtraIntr, name: 'extrav_introver_score' },
          { param: countNeiro, name: 'neirotizm_score' },
          { param: countLie, name: 'sincerity_score' },
        ]
      };
