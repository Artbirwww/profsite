...
export const simulationAPI = {
    ...
    getSimulationsPageable: async (request: SimulationRequest, token:string, signal?: AbortSignal) => {
        ...
        if (request.email) 
            params.append('email', encodeURIComponent(request.email))
        if (request.profession) 
            params.append('profession', encodeURIComponent(request.profession))
        if (request.simulationType) 
            params.append('simulationType', encodeURIComponent(request.simulationType))
